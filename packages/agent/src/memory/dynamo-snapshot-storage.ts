...
// interface (@strands-agents/sdk). Passed to a `SessionManager`, it persists
// agent state snapshots so a conversation resumes across WebSocket
// connections and cold starts — the job AgentCore Memory did for the Python
// agent, now owned by us.
//
// A SnapshotLocation is { sessionId, scope, scopeId }. All three partition a
// snapshot, so the session id alone keys the DynamoDB partition and the
// scope/scopeId ride in the sort key. deleteSession (which receives only a
// sessionId) can then wipe every scope with a single partition query.
//
