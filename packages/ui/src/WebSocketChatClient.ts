import type { ServerMessage, ServerMessageListener } from './protocol.js';

// Framework-agnostic WebSocket client for the AgentCore chat stream. Ported
// from the original frontend services/websocket.ts, with one change: it no
// longer imports app-specific auth/api services. Instead the caller injects a
// `getConnectionUrl` function that returns a presigned wss:// URL — so the
// client stays reusable across apps (this is the shape that migrates into
// @readysetcloud/ui).
