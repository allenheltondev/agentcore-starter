...
import { WebSocketChatClient } from './WebSocketChatClient.js';
import type { ConnectionStatus, ServerMessage } from './protocol.js';

// useAgentChat merges what were three files in the original frontend — the
// WebSocketContext (connection lifecycle + reconnect/backoff), the
// WebSocketService (transport), and the Chat component's streaming state — into
// one reusable hook. This is the surface that migrates into @readysetcloud/ui:
// an app supplies `getConnectionUrl` (which calls the backend presigned-URL
// endpoint with its auth) and a `sessionId`, and gets back everything needed to
